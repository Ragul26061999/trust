...
export default function ProtectedLayout({
  ...
  // If not authenticated and not loading, don't render anything (redirect is happening)
  if (!loading && !user) {
    return (
      <AppThemeProvider>
        <MuiThemeProvider theme={muiTheme}>
          <Box 
            sx={{ 
              ...
              backgroundImage: userTheme.backgroundImage ? `url(${userTheme.backgroundImage})` : 'none',
  ...
  return (
    <AppThemeProvider>
      <MuiThemeProvider theme={muiTheme}>
        {showSidebar ? (
          <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: 'background.default', backgroundImage: userTheme.backgroundImage ? `url(${userTheme.backgroundImage})` : 'none', backgroundSize: 'cover', backgroundPosition: 'center' }}>
            <Sidebar />
          ...
          <Box sx={{ minHeight: '100vh', bgcolor: 'background.default', backgroundImage: userTheme.backgroundImage ? `url(${userTheme.backgroundImage})` : 'none', backgroundSize: 'cover', backgroundPosition: 'center' }}>
            <Box component="main" sx={{ p: 4, bgcolor: 'background.default' }}>
              {children}
            </Box>
          </Box>
        )}
